...
internal class Program
{
		...
		while (!avslutt)
		{
			...
			switch (valg)
			{
				...
				case "0":
					...
					break;
				default:
					Console.WriteLine("Ugyldig valg, prov igjen.");
					break;
			}
	...
	{
		Studenter.Add(new Student(1001, "Ola Nordmann", "ola@uia.no"));
		Studenter.Add(new Student(1002, "Kari Hansen", "kari@uia.no"));
		...
		Ansatte.Add(new Ansatt(501, "Lars Berg", "lars@uia.no", "Foreleser", "Informatikk"));
		Ansatte.Add(new Ansatt(502, "Anne Lie", "anne@uia.no", "Bibliotekar", "Bibliotek"));

		KursListe.Add(new Kurs("IS-110", "Programmering", 10, 2));
		KursListe.Add(new Kurs("IS-120", "Databaser", 10, 3));

		Bibliotek.RegistrerBok(new Bok(1, "Ren kode", "Robert C. Martin", 2008, 3));
		Bibliotek.RegistrerBok(new Bok(2, "Design Patterns", "GoF", 1994, 2));
	}
	...
	private static void VisLanOversikt()
	{
		...
		{
			Console.WriteLine("Ingen historikk ennå.");
			return;
		}
	...
	private static int LesInt(string prompt)
	{
		while (true)
		{
			...
			Console.WriteLine("Ugyldig tall, prov igjen.");
		}
	}
}
